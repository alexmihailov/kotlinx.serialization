...
class HoconJDurationTest {
    ...
    fun testDeserializeListOfDuration() {
        ...
        Assert.assertEquals(listOf(ofDays(1), ofMinutes(1), ofNanos(5)), obj.ld)
    }
    ...
    fun testDeserializeMapOfDuration() {
        ...
        Assert.assertEquals(mapOf("day" to ofDays(2), "hour" to ofHours(5), "minute" to ofMinutes(3)), obj.mp)

        val objDurationKey = deserializeConfig("""
             mp: { 1 hour = 3600s }
        """.trimIndent(), ConfigMapDurationKey.serializer())
        Assert.assertEquals(mapOf(ofHours(1) to ofSeconds(3600)), objDurationKey.mp)
    }
    ...
    fun testDeserializeComplexDuration() {
        ...
        Assert.assertEquals(ofMinutes(5), obj.s.d)
        Assert.assertNull(obj.n.d)
        Assert.assertEquals(listOf(Simple(ofMinutes(1)), Simple(ofSeconds(2))), obj.l)
        Assert.assertEquals(listOf(Nullable(null), Nullable(ofHours(6))), obj.ln)
        Assert.assertEquals(6, obj.i)
        Assert.assertTrue(obj.f)
        Assert.assertEquals(listOf(ofDays(1), ofMinutes(1), ofNanos(5)), obj.ld)
        Assert.assertEquals(mapOf("day" to ofDays(2), "hour" to ofHours(5), "minute" to ofMinutes(3)), obj.mp)
        Assert.assertEquals(mapOf(ofHours(1) to ofSeconds(3600)), obj.mpp)
    }
    ...
    @Test
    fun testThrowsWhenUseJDurationSerializer() {
        val message = "JDurationSerializer only supports HOCON and should not be used outside of it."
        assertFailsWith<SerializationException>(message) {
            val fakeEncoder = object : AbstractDecoder() {
                override val serializersModule: SerializersModule
                    get() = TODO()
                override fun decodeElementIndex(descriptor: SerialDescriptor): Int = TODO()

            }
            JDurationSerializer.deserialize(fakeEncoder)
        }
        assertFailsWith<SerializationException>(message) {
            val fakeEncoder = object : AbstractEncoder() {
                override val serializersModule: SerializersModule
                    get() = TODO()
            }
            JDurationSerializer.serialize(fakeEncoder, ofMinutes(1))
        }
    }
}
